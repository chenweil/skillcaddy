...
import { buildCollectionSetupAdvice, scanCollectionSetups } from './collectionSetup.js';
import { getHermesSkillsDir, isHermesEligibleSource, scanHermesSkills } from './hermesStore.js';
...
export async function getState(rootDir, projectPath, { readOnly = false, globalDir, hermesDir } = {}) {
  const safeProjectPath = normalizeProjectPath(projectPath);
  if (!readOnly) await ensureSourceFolders(rootDir);
  const skills = await scanSkills(rootDir);
  const enabled = await scanEnabledSkills(safeProjectPath);
  const global = (await scanGlobalSkills({ globalDir })).map((skill) => ({
    ...skill,
    canDisable: skill.isSymlink && isInsideAllowedSkillSources(rootDir, skill.targetPath)
  }));
  const hermes = (await scanHermesSkills({ hermesDir })).map((skill) => ({
    ...skill,
    canDisable: skill.isSymlink && isInsideAllowedSkillSources(rootDir, skill.targetPath)
  }));
  const setups = await scanCollectionSetups(rootDir, safeProjectPath, skills, enabled);
  const advice = buildSkillAdvice(rootDir, { skills, enabled, global, hermes, setups });
  const sources = await readSourceRecords(rootDir);
  ...
  return {
    ...
    setups,
    advice,
    sources: buildSourcesInfo(sources),
...
export async function scanGlobalSkills({ globalDir } = {}) {
  const directory = getGlobalSkillsDir(globalDir);
  const skills = await scanSkillLinks(directory);
  return skills
    .map((skill) => ({ ...skill, agent: 'Agents', scope: 'global', directory }))
    .sort((left, right) => left.alias.localeCompare(right.alias));
}

function buildSkillAdvice(rootDir, { skills, enabled, global, hermes, setups }) {
  const advice = buildCollectionSetupAdvice(setups);
  ...
  const hermesByAlias = groupBy(hermes, (skill) => skill.alias);
  ...
  for (const skill of hermes) {
    const matchingSource = skills.find((candidate) => candidate.path === skill.targetPath);
    const managedTarget = skill.isSymlink && isInsideAllowedSkillSources(rootDir, skill.targetPath);
    if (!matchingSource && !managedTarget) continue;

    if (!skill.isSymlink) {
      advice.push({
        severity: 'warning',
        type: 'hermes-unmanaged-entry',
        title: `Hermes 存在非软链接 skill：${skill.alias}`,
        detail: `${skill.linkPath} 不是 Skillcaddy 可安全清理的软链接。启用同名 Hermes skill 前请先确认该条目用途。`,
        alias: skill.alias,
        path: skill.linkPath,
        scope: 'hermes'
      });
    ...
    if (!skill.exists) {
      advice.push({
        severity: 'warning',
        type: 'hermes-broken-link',
        title: `Hermes skill 断链：${skill.alias}`,
        detail: `${skill.linkPath} 指向不存在的目标：${skill.targetPath}`,
        alias: skill.alias,
        path: skill.linkPath,
        targetPath: skill.targetPath,
    ...
    if (!isInsideAllowedSkillSources(rootDir, skill.targetPath)) {
      advice.push({
        severity: 'info',
        type: 'hermes-unmanaged-symlink',
        title: `Hermes 存在不纳管 skill：${skill.alias}`,
        detail: `${skill.alias} 指向当前 Skillcaddy 来源目录之外：${skill.targetPath}`,
        alias: skill.alias,
        path: skill.linkPath,
        targetPath: skill.targetPath,
        scope: 'hermes'
...
export async function enableSkill(rootDir, input) {
  const scope = normalizeEnablementScope(input.scope);
  ...
  assertHermesEligible(rootDir, resolvedSkillPath, scope);

  const skillsDir = getScopeSkillsDir(scope, input);
  ...
  if (await linkPathExists(linkPath)) {
    ...
    return { ok: true, scope, alias, linkPath, targetPath: resolvedSkillPath, unchanged: true };
  }

  await symlink(resolvedSkillPath, linkPath, 'dir');
  return { ok: true, scope, alias, linkPath, targetPath: resolvedSkillPath, unchanged: false };
}

export async function disableSkill(input) {
  const scope = normalizeEnablementScope(input.scope);
  const alias = normalizeAlias(input.alias);
  const linkPath = path.join(getScopeSkillsDir(scope, input), alias);

  if (!(await linkPathExists(linkPath))) {
    return { ok: true, scope, alias, linkPath, removed: false };
  }
  ...
  if (scope === 'global' || scope === 'hermes') {
    if (!input.rootDir || typeof input.rootDir !== 'string') {
      throw new Error(`${scope} disable requires rootDir`);
    }
    const targetPath = await resolveLinkTarget(linkPath);
    if (!isInsideAllowedSkillSources(path.resolve(input.rootDir), targetPath)) {
      throw new Error(`拒绝清理不属于当前 Skillcaddy 原件库的 ${scope} 软链接：${targetPath}`);
    }
  }

  await rm(linkPath);
  return { ok: true, scope, alias, linkPath, removed: true };
}

export function getGlobalSkillsDir(globalDir) {
  return path.resolve(globalDir || path.join(homedir(), '.agents', 'skills'));
}

function getScopeSkillsDir(scope, input) {
  if (scope === 'global') return getGlobalSkillsDir(input.globalDir);
  if (scope === 'hermes') return getHermesSkillsDir(input.hermesDir);
  return getAgentsSkillsDir(normalizeProjectPath(input.projectPath));
}

function assertHermesEligible(rootDir, skillPath, scope) {
  if (scope !== 'hermes') return;
  const source = resolveAllowedSkillSource(rootDir, skillPath);
  if (!source || !isHermesEligibleSource(source.sourceFolder)) {
    throw new Error('Hermes 只允许启用 official、github 或 personal 来源的 skill');
  }
}
...
async function scanSourceEntry(rootDir, source, sourceDir, entryName) {
  ...
  if (await sourcePublicationInProgress(entryPath)) return [];
  const skillDirs = await discoverSourceSkillDirectories(entryPath);
  if (await sourcePublicationInProgress(entryPath)) return [];
  return Promise.all(
...
async function sourcePublicationInProgress(sourcePath) {
  try {
    return await readFile(
      path.join(sourcePath, SOURCE_INSTALLING_MARKER),
      'utf8'
    ) === SOURCE_INSTALLING_MARKER_CONTENT;
  } catch {
    return false;
...
function buildSourcesInfo(records) {
  return records.map((record) => {
    ...
    if (record.type === 'git' && record.origin?.commit) {
      info.version = record.origin.commit.slice(0, 7);
    } else if (record.integrity?.value) {
      info.version = record.integrity.value.slice(0, 12);
      info.algorithm = record.integrity.algorithm;
    }
