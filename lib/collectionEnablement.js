...
export async function executeCollectionEnablement(rootDir, input) {
  ...
  const state = await getState(rootDir, projectPath, { globalDir: input.globalDir });
  ...
  const refreshed = scope === 'global'
    ? { setups: [], reminders: [], refresh: { ok: true } }
    : await refreshSetupGuidance(rootDir, state.projectPath, plan.setups, input.globalDir);
  return {
    scope,
    plan,
    outcomes,
...
async function refreshSetupGuidance(rootDir, projectPath, plannedSetups, globalDir) {
  try {
    const state = await getState(rootDir, projectPath, { globalDir });
