...
import { validateRemoteSkillFileUrl } from './sourceRemoteFilePolicy.js';

const SUPPORTED_SCHEMA_VERSION = 1;
const SOURCE_BUCKET_BY_TYPE = {
  git: 'github',
  archive: 'official',
  'remote-file': 'official',
  local: 'personal'
};
...
export function validateSourceRecord(rootDir, record, expectedSourceId) {
  ...
  const skills = validateSkillPaths(record.skills);
  const installPath = validateInstallPath(rootDir, record.installPath);
  if (record.type === 'remote-file' && !integrity) {
    throw new Error('Remote file source record requires integrity');
  }
  if (
    record.type === 'remote-file' &&
    (skills.length !== 1 || skills[0] !== '.')
  ) {
    throw new Error('Remote file source record requires exactly the root skill');
  }
  if (
    record.type === 'remote-file' &&
    (sourceId.split('/').length !== 2 || installPath !== sourceId)
  ) {
    throw new Error(
      'Remote file source install path must match its official source identity'
    );
  }
  return {
    ...
    ...(record.focus === undefined ? {} : { focus: validateSourceFocus(record.type, record.focus) }),
    ...(integrity ? { integrity } : {}),
    skills
  };
}

function validateSourceFocus(type, focus) {
  if (type !== 'git' || !focus || typeof focus !== 'object' || Array.isArray(focus)) {
    throw new Error('Source focus is only valid for Git sources');
  }
  const ref = validateGitRef(focus.ref);
  if (!ref) throw new Error('Git source focus requires a ref');
  const allowedKeys = new Set(['ref', 'path']);
  if (Object.keys(focus).some((key) => !allowedKeys.has(key))) {
    throw new Error('Git source focus contains unsupported fields');
  }
  if (focus.path === undefined) return { ref };
  if (
    typeof focus.path !== 'string' ||
    !focus.path ||
    path.posix.isAbsolute(focus.path) ||
    path.posix.normalize(focus.path) !== focus.path ||
    focus.path.includes('\\') ||
    hasControlCharacters(focus.path) ||
    focus.path.split('/').some((segment) => !segment || segment === '.' || segment === '..')
  ) {
    throw new Error('Invalid Git source focus path');
  }
  return { ref, path: focus.path };
}

function validateInstallPath(rootDir, installPath) {
  if (
...
function validateOrigin(type, origin) {
  ...
  if (type === 'git') {
    ...
    return compactObject({
      kind: 'git',
      remote: sanitizeGitRemote(origin.remote),
      ref: validateGitRef(origin.ref),
      commit: optionalCommit(origin.commit)
    });
  }

  if (type === 'remote-file') {
    if (!['http', 'https'].includes(origin.kind) || typeof origin.display !== 'string') {
      throw new Error('Remote file source origin requires a stable HTTP(S) URL');
    }
    let parsed;
    try {
      parsed = validateRemoteSkillFileUrl(origin.display);
    } catch (error) {
      throw new Error(`Remote file source origin is invalid: ${error.message}`);
    }
    if (parsed.protocol !== origin.kind) {
      throw new Error('Remote file source origin kind must match its URL protocol');
    }
    return { kind: origin.kind, display: parsed.display };
  }

  if (type === 'archive') {
    if (!['http', 'https'].includes(origin.kind) || typeof origin.display !== 'string' || !origin.display.trim()) {
      throw new Error('Archive source origin requires an HTTP(S) display URL');
    }
    const display = sanitizeUrl(origin.display, [`${origin.kind}:`]);
    return { kind: origin.kind, display };
  }
...
export function sanitizeGitRemote(remote) {
  ...
  if (trimmed.includes('://')) return sanitizeUrl(trimmed, ['https:', 'ssh:']);
  if (!/^(?:[^@\s]+@)?[^:/\s]+:[^?#\s]+(?:[?#].*)?$/.test(trimmed)) {
    throw new Error('Invalid Git source origin');
  }
  return trimmed
    .replace(/^[^@\s]+@(?=[^:\s]+:)/, '')
    .replace(/[?#].*$/, '');
...
function validateGitRef(value) {
  ...
  if (
    ...
    throw new Error('Invalid origin.ref');
  }
  return value;
}
