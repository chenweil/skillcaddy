...
const reportedExportErrors = new WeakSet();
...
function failExportCheck(context, check, message) {
  reportImage(context, `[fail] ${check}: ${message}`);
  const error = imageError('export-blocked', `${check}: ${message}`);
  reportedExportErrors.add(error);
  throw error;
}
function reportAndThrowExportFailure(context, check, error) {
  const wrapped = wrapImageError(error, 'export-blocked');
  reportImage(context, `[fail] ${check}: ${wrapped.message}`);
  reportedExportErrors.add(wrapped);
  throw wrapped;
}
...
async function checkExport(context) {
  ...
  if (await sourcePathExists(managedRoot)) {
    ...
    if (!managedInfo.isDirectory() || managedInfo.isSymbolicLink() ||
      ...
      failExportCheck(context, 'managed-state', 'The .skillcaddy state directory must be a directory inside the central-library root.');
    }
  }
  const items = [];
  for (const bucket of SOURCE_FOLDERS) {
    if (await sourcePathExists(path.join(rootDir, bucket))) {
      ...
      if (!info.isDirectory()) failExportCheck(context, 'registered-bucket-layout', `Unsafe bucket: ${bucket}`);
      items.push(...await imageTree(rootDir, bucket));
    }
  }
  if (items.some(item => path.basename(item.path) === SOURCE_INSTALLING_MARKER)) {
    failExportCheck(context, 'installing-markers', 'Installing marker found. Recover the interrupted source acquisition before export.');
  }
  ...
  if (await sourcePathExists(stagingPath)) {
    failExportCheck(context, 'staging-residue', 'Staging residue found. Inspect .skillcaddy/staging/ and finish or cancel the interrupted transaction.');
  }
  ...
  for (const item of items) {
    if (!belongsToRegisteredSource(item, records)) {
      failExportCheck(context, 'registered-bucket-layout', `Unregistered bucket entry: ${item.path}. Register or move it before export.`);
    }
  }
  ...
  for (const record of records) {
    if (!await sourcePathExists(path.join(rootDir, record.installPath)) ||
      ...
      failExportCheck(context, 'registered-directories', `Missing registered directory: ${record.installPath}. Restore it before export.`);
    }
  }
  ...
  for (const record of records) {
    ...
    if (record.type === 'git') headRecords.push({ sourceId: record.sourceId, head: await readGitHead(source) });
      ...
      failExportCheck(context, 'source-integrity', `Integrity drift: ${record.sourceId}. Review and repair the source before export.`);
    }
    else if (!record.integrity) {
  ...
  catch { failExportCheck(context, 'repository-state', 'Repository state unavailable. Commit on a branch and configure its upstream before export.'); }
  if (producer.dirty) failExportCheck(context, 'repository-state', 'Uncommitted changes found. Commit them before export.');
  if (!producer.pushed) failExportCheck(context, 'repository-state', 'Unpushed commits found. Push them before export.');
  reportImage(context, '[pass] repository state');
  for (const item of items) {
    if (item.info.mode & 0o7000) failExportCheck(context, 'entry-safety', `Privileged mode: ${item.path}`);
    if (item.info.isSymbolicLink()) {
      ...
      if (path.isAbsolute(target) || !isPathInsideOrEqual(rootDir, resolved)) failExportCheck(context, 'entry-safety', `Escaping symlink: ${item.path}`);
      try {
        if (!isPathInsideOrEqual(await realpath(rootDir), await realpath(path.join(rootDir, item.path)))) failExportCheck(context, 'entry-safety', `Escaping symlink: ${item.path}`);
      } catch (error) {
        if (error.code !== 'ENOENT') throw error;
        reportImage(context, `[warn] dead-symlink: ${item.path}`);
      }
    } else if (!item.info.isDirectory() && !item.info.isFile()) failExportCheck(context, 'entry-safety', `Special file: ${item.path}`);
  }
...
export async function exportLibraryImage(context, filePath) {
  ...
  let temporary;
  try {
    if (await sourcePathExists(destination)) failExportCheck(context, 'destination', 'Library image already exists. Choose a new .tar.gz destination.');
    ...
    if (result.code !== 0) failExportCheck(context, 'packing', `Packing failed: ${result.stderr}`);
    try {
      await verifyPackedImage(temporary, packing, workspace.root, context);
    } catch (error) {
      reportAndThrowExportFailure(context, 'post-pack-verification', error);
    }
    for (const record of before.records) {
      if (await checksumDirectory(path.join(context.rootDir, record.installPath)) !== before.hashes[record.sourceId] ||
        ...
        failExportCheck(context, 'post-pack-drift', `Source drift: ${record.sourceId}. Retry once the library is idle.`);
      }
    }
    for (const bucket of SOURCE_FOLDERS) {
      if (await checksumOptionalDirectory(path.join(context.rootDir, bucket)) !== before.bucketHashes[bucket]) {
        failExportCheck(context, 'post-pack-drift', `Source tree drift: ${bucket}. Retry once the library is idle.`);
      }
    }
    if (JSON.stringify(await readSourceRecords(context.rootDir)) !== JSON.stringify(before.records) ||
      ...
      failExportCheck(context, 'post-pack-drift', 'Registry, metadata, enablement, or repository drift. Retry once the library is idle.');
    }
    ...
    catch (error) { if (error.code === 'EEXIST') failExportCheck(context, 'destination', 'Library image destination became occupied. Choose a new path.'); throw error; }
    return { path: destination, manifest };
  } catch (error) {
    const wrapped = wrapImageError(error, 'export-blocked');
    if (!reportedExportErrors.has(wrapped)) {
      reportImage(context, `[fail] export: ${wrapped.message}`);
    }
    throw wrapped;
  }
  finally {
  ...
  for (const record of records) {
    ...
    if (record.type === 'git') {
      ...
      if (!head || await readGitHead(source) !== head) throw imageError('source-validation', `Staged Git HEAD mismatch: ${record.sourceId}`);
        ...
        await checksumDirectory(source) !== record.integrity.value &&
        await checksumDirectoryNormalized(source) !== record.integrity.value) {
      // SPEC:127 compares the plain checksumDirectory like-for-like with the baseline, but
      // ADR 0011:117-119 promises an NFD/NFC naming artifact never blocks import. Accept when
      // EITHER the plain or the NFC-normalized checksum matches the baseline (same tolerance the
      // export pre-flight uses in checkExport); only a difference under BOTH — real byte or
      // structural drift, which normalization cannot mask — is a hard failure.
...
function dependencySafeSourceOrder(records) {
  // Phase one rejects cross-source symlinks, and source records do not carry
  // another dependency edge. Every accepted source therefore has no dependents;
  // sourceId order provides a stable dependency-safe submission order.
...
export async function importLibraryImage(context, filePath, options = {}) {
  ...
  try {
    ...
    const records = dependencySafeSourceOrder(
      await verifyImageSources(staged, manifest, entries)
    );
