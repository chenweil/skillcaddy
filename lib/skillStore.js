...
export async function getState(rootDir, projectPath, { readOnly = false, globalDir } = {}) {
  ...
  const advice = buildSkillAdvice(rootDir, { skills, enabled, global, setups });
...
function buildSkillAdvice(rootDir, { skills, enabled, global, setups }) {
...
export async function enableSkill(rootDir, input) {
  ...
  const skillsDir = scope === 'global'
    ? getGlobalSkillsDir(input.globalDir)
    : getAgentsSkillsDir(normalizeProjectPath(input.projectPath));
...
export async function disableSkill(input) {
  ...
  if (scope === 'global') {
    if (!input.rootDir || typeof input.rootDir !== 'string') {
      throw new Error('global disable requires rootDir');
    }
    const targetPath = await resolveLinkTarget(linkPath);
    if (!isInsideAllowedSkillSources(path.resolve(input.rootDir), targetPath)) {
      throw new Error(`拒绝清理不属于当前 Skillcaddy 原件库的全局软链接：${targetPath}`);
    }
  }
