...
async function buildSourceUpgradePlan(context, request) {
  ...
  const affectedGlobalLinks = await findAffectedGlobalLinks({
    rootDir: context.rootDir,
    globalDir: context.globalDir,
    installPath: record.installPath,
    removedSkillPaths: changes.removedOrRelocated
  });
  const affectedHermesLinks = await findAffectedHermesLinks({
    rootDir: context.rootDir,
...
function buildUpgradePlan({
  ...
  return {
    ...
    ...(candidate.skillChanges ? { skillChanges: candidate.skillChanges } : {}),
    changes,
    affectedProjectLinks,
...
function buildUpdateResult(plan, status = plan.status) {
  if (plan.input.type === 'git') {
    return {
      ...
      ...(plan.skillChanges ? { skillChanges: plan.skillChanges } : {}),
