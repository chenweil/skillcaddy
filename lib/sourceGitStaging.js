...
export async function cloneAndInspectGitSource(stagingRoot, parsed) {
  ...
  if (parsed.ref) cloneArguments.push('--branch', parsed.ref);
  cloneArguments.push('--', parsed.cloneRemote, contentRoot);
...
function safeGitError(error) {
  const message = typeof error?.stderr === 'string' && error.stderr.trim()
    ? error.stderr.trim()
    : error.message;
  return String(message).replace(/(?:https|ssh):\/\/[^\s'"]+/gi, '[sanitized Git URL]');
}
