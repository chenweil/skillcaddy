...
export async function validateSourceTree(rootDir, sourcePath) {
  ...
  if (!isPathInsideOrEqual(resolvedRoot, resolvedSource)) {
...
export async function checksumDirectory(directory) {
  return checksumDirectoryWithOptions(directory, false);
}

/**
 * Compute the image-side checksum after applying the NFC path policy from
 * ADR-0011/#35. The ordinary checksum remains unchanged for existing source
 * acquisition and registry contracts.
 */
export async function checksumDirectoryNormalized(directory) {
  return checksumDirectoryWithOptions(directory, true);
}

async function checksumDirectoryWithOptions(directory, normalizePaths) {
  const hash = createHash('sha256');

  async function walk(currentPath) {
    const entries = await readdir(currentPath, { withFileTypes: true });
    for (const entry of entries.sort((left, right) => {
      const leftName = normalizePaths ? left.name.normalize('NFC') : left.name;
      const rightName = normalizePaths ? right.name.normalize('NFC') : right.name;
      ...
      const relativePath = path.relative(directory, entryPath)
        .split(path.sep)
        .join('/');
      const hashedPath = normalizePaths ? relativePath.normalize('NFC') : relativePath;
      if (entry.isDirectory()) {
        hash.update(`directory\0${hashedPath}\0`);
        await walk(entryPath);
      } else if (entry.isFile()) {
        hash.update(`file\0${hashedPath}\0`);
        hash.update(await readFile(entryPath));
        hash.update('\0');
      } else if (entry.isSymbolicLink()) {
        const target = await readlink(entryPath);
        hash.update(`symlink\0${hashedPath}\0${normalizePaths ? target.normalize('NFC') : target}\0`);
      } else {
        throw new Error(`Unsupported source entry: ${hashedPath}`);
      }
    }
  }
...
export async function sourceInstallPathAvailable(rootDir, records, installPath) {
...
export async function ensureSourceDirectory(rootDir, ...segments) {
  ...
  for (const segment of segments) {
    ...
    if (!isPathInsideOrEqual(resolvedRoot, current)) {
...
async function validateSymlink(resolvedSource, entryPath, relativePath) {
  ...
  if (!isPathInsideOrEqual(resolvedSource, resolvedTarget)) {
