...
// 会进一步收紧或补充；T4 以工程上可验证的最小集落地。
//
// 起始值依据：本机 macOS bsdtar 3.5.3 的实测：
//   --no-acls  ✓      --no-xattrs       ✓      --no-same-permissions  ✓
//   --no-mac-metadata ✓ (bsdtar)        --no-fflags  ✓ (bsdtar)
//   --no-selinux ✗ (GNU tar)           --no-overwrite-dir ✗ (GNU tar)
//
// GNU tar manual (1.35) admits the GNU-only flags; verifying against a Linux
// runtime is part of #33's research ticket. Until then, `gnutar` stays empty
// and the runtime keeps bsdtar's set instead of crashing on unknown flags.
...
async function probeTarImplementation(tarPath) {
  const { code, stdout, stderr } = await spawnTar(['--version'], { tarPath });
  if (code !== 0) {
    throw new SourceAcquisitionError(
      'source-safety',
  ...
  if (/GNU tar|tar \(GNU tar\)/i.test(firstLine)) {
    return { implementation: 'gnutard', version: firstLine };
  }
  throw new SourceAcquisitionError(
    'source-safety',
    `unknown tar implementation: ${firstLine || '<empty>'}`
  );
}
...
function flagsForImplementation(implementation) {
  ...
  if (implementation === 'gnutard') return [...policy.common, ...policy.gnutar];
  return [...policy.common];
}

/**
 * 探测系统 tar 的实现，并在允许列表内选择合适的 flag 集合。
 * 探测失败或实现未被识别时一向拒收：不识别实现就意味着后续 flag
 * 可能会被 GNU tar 拒收（不可能安全使用），唯一合理的做法是发高亮。
 */
export async function resolveTarInvocation(tarPath = 'tar') {
  const probe = await probeTarImplementation(tarPath);
  return {
    binary: tarPath,
...
export async function extractLibraryImage(filePath, stagingRoot, overrides = {}) {
  ...
  try {
    ...
    const extractArgs = composeExtractArgs({ invocation, filePath, stagingRoot });
