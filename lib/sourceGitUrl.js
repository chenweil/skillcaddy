...
function validateParsedGitSource(parsed) {
  ...
  if (parsed.ref) validateGitRef(parsed.ref);
...
function validateGitRef(ref) {
  if (
    ...
    ref.endsWith('/') ||
    ref.endsWith('.') ||
    ref.split('/').some((segment) => segment.startsWith('.') || segment.endsWith('.lock'))
  ) {
    throw identityError('GitHub tree URL contains an invalid branch');
  }
}
