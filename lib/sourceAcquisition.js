  ...
  return defineSourceAcquisitionAdapter({
    ...
    async assertInstalled(rootDir, existing, plan) {
      ...
      if (!await sourcePathExists(installed)) {
        throw new SourceAcquisitionError('image-source-mismatch', `Existing source is missing: ${record.sourceId}.`);
      }
      if (record.type === 'git') {
        let head;
        try { head = await readGitHead(installed); } catch {
          throw new SourceAcquisitionError('image-source-mismatch', `Existing Git source cannot report HEAD: ${record.sourceId}.`);
        }
        if (!plan.input.head || head !== plan.input.head) {
          throw new SourceAcquisitionError('image-source-mismatch', `Existing Git source has a different HEAD: ${record.sourceId}. Import never replaces source bytes.`);
        }
        return;
      }
      if (await checksumDirectoryNormalized(installed) !== plan.input.digest) {
        throw new SourceAcquisitionError('image-source-mismatch', `Existing source differs: ${record.sourceId}. Import never replaces source bytes.`);
      }
    }
  });
}
