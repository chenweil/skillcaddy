...
export async function migrateLegacySkillMetadata(rootDir, skills, options = {}) {
  const inspected = await Promise.all(skills.map(async (skill) => {
    if (skill.hasMetadata || skill.metadataError) return null;

    const legacyPath = getLegacyMetadataPath(skill.path);
    try {
      const metadata = normalizeMetadata(JSON.parse(await readFile(legacyPath, 'utf8')));
      return {
        item: {
          id: skill.id,
      ...
      return {
        invalid: {
          id: skill.id,
          metadataError: `legacy skillcaddy.json 读取失败：${error.message}`
        }
      };
    }
  }));

  const candidates = inspected.filter((entry) => entry?.item);
  const pending = candidates.map((entry) => entry.item);
  const invalid = inspected.filter((entry) => entry?.invalid).map((entry) => entry.invalid);

  if (!options.write) {
    return { dryRun: true, pending, migrated: 0, invalid };
  }

  for (const candidate of candidates) {
    await updateSkillMetadata(rootDir, {
      skillPath: candidate.item.skillPath,
      ...candidate.metadata
    });
  }

  return { dryRun: false, pending, migrated: pending.length, invalid };
}
