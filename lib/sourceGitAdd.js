...
async function installPreparedDirectory(preparedRoot, destination) {
  ...
  try {
    const entries = await readdir(preparedRoot, { withFileTypes: true });
    entries.sort((left, right) =>
      visibilityOrder(left.name) - visibilityOrder(right.name) ||
...
function visibilityOrder(entryName) {
  return entryName === 'SKILL.md' || entryName === 'skills' ? 1 : 0;
}
