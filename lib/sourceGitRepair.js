...
async function buildRepairPlan(context, request) {
  ...
  const candidate = await inspectGitRegistryRepair(context, record);
  ...
  const affectedHermesLinks = candidate.status === 'dirty'
    ? []
    : await findAffectedHermesLinks({
        rootDir,
  ...
  return {
    ...
    plan: {
      ...
      ...(affectedHermesLinks.length > 0 ? { affectedHermesLinks } : {})
    }
  };
}
...
function requireAuthorization(authorization) {
  ...
  if (authorization?.kind === 'ordinary') {
      return {
        enforce: (plan) => assertNoAffectedLinks(
