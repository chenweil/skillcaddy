...
const REPOSITORY_SKILLS_SOURCE = 'local';
const REPOSITORY_SKILLS_FOLDER = 'skills';
...
function buildSkillAdvice(rootDir, { skills, enabled, global }) {
  ...
  for (const [name, matches] of availableByName) {
    ...
    advice.push({
      ...
      detail: `启用时需要按来源确认，避免默认 alias ${name} 指向非预期 skill。`,
      alias: name,
...
function isInsideAllowedSkillSources(rootDir, skillPath) {
