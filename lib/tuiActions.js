...
import { executeCollectionEnablement } from './collectionEnablement.js';
...
export async function loadTuiState(rootDir, projectPath, options = {}) {
  return getState(rootDir, projectPath, options);
}
...
export function listSkillChoices(state, options = {}) {
  ...
  const enabledTargets = new Set(
    enabledSkillsForScope(state, options.scope)
...
export function listEnabledAliases(state, scope = 'project') {
  const enabled = enabledSkillsForScope(state, scope);
  return enabled.map((skill, index) => ({
    index: index + 1,
    alias: skill.alias,
    label: `${skill.alias} -> ${skill.targetPath || skill.linkPath}`,
    canDisable: skill.canDisable ?? skill.isSymlink,
    skill
  }));
}
...
export async function enableSkillChoice(rootDir, state, value, options = {}) {
  ...
  return enableProjectSkill(rootDir, {
    ...(options.scope === 'project' || !options.scope ? { projectPath: state.projectPath } : {}),
    ...(options.scope ? { scope: options.scope } : {}),
    ...(options.globalDir ? { globalDir: options.globalDir } : {}),
    ...(options.hermesDir ? { hermesDir: options.hermesDir } : {}),
    skillPath: skill.path,
    alias: options.alias || skill.name
  });
}

export async function enableCollectionChoice(rootDir, state, skillIds, options = {}) {
  return executeCollectionEnablement(rootDir, {
    ...(options.scope === 'project' || !options.scope ? { projectPath: state.projectPath } : {}),
    ...(options.scope ? { scope: options.scope } : {}),
    ...(options.globalDir ? { globalDir: options.globalDir } : {}),
    ...(options.hermesDir ? { hermesDir: options.hermesDir } : {}),
...
export async function disableAlias(state, alias, options = {}) {
  return disableSkill({
    ...(options.scope === 'project' || !options.scope ? { projectPath: state.projectPath } : {}),
    ...(options.scope ? { scope: options.scope } : {}),
...
function enabledSkillsForScope(state, scope = 'project') {
