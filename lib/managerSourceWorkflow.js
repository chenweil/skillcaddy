...
async function planEnablement(context, acquisition, request) {
  ...
  const state = await getState(context.rootDir, requireProjectPath(context));
  return buildSkillEnablePlan(state, request.skillId, request.alias);
}
...
async function applyEnablement(context, acquisitionPlan, request) {
  ...
  const skillId = acquisitionPlan
    ...
    : request.skillId;
  const enablementPlan = buildSkillEnablePlan(state, skillId, request.alias);
  const result = await enableProjectSkill(context.rootDir, {
    projectPath: context.projectPath,
    skillPath: enablementPlan.skillPath,
    alias: enablementPlan.alias
  });
  const verifiedState = await getState(context.rootDir, context.projectPath);
