...
export async function resolveSourceAcquisitionStatus({
  ...
  if (sameIdentity) {
    ...
    // A library image is allowed to repair a receiver whose registry record
    // survived while its source directory did not. Keep the existing sidecar
    // and restore only the missing bytes; ordinary acquisition inputs retain
    // their existing collision semantics.
    if (
      plan.input?.type === 'image' &&
      !await sourcePathExists(resolveSourceInstallDestination(rootDir, sameIdentity.installPath))
    ) {
      return 'ready';
    }
