...
export async function writeSourceRecord(rootDir, record) {
  return publishSourceRecord(rootDir, record, replaceRecord);
}

export async function createSourceRecord(rootDir, record) {
  return publishSourceRecord(rootDir, record, createRecord);
}

async function publishSourceRecord(rootDir, record, publish) {
  const staged = await stageSourceRecord(rootDir, record);
  let published = false;
  try {
    await publish(staged.temporaryPath, staged.targetPath);
    published = true;
  } finally {
    try {
      await discardStagedSourceRecord(staged);
    } catch (error) {
...
export async function stageSourceRecord(rootDir, record) {
...
export async function discardStagedSourceRecord(staged) {
  await rm(staged.temporaryPath, { force: true });
}

export async function publishStagedSourceRecord(staged) {
  await replaceRecord(staged.temporaryPath, staged.targetPath);
}

async function replaceRecord(temporaryPath, targetPath) {
  await rename(temporaryPath, targetPath);
}

async function createRecord(temporaryPath, targetPath) {
...
async function assertExistingPathInsideRoot(rootDir, targetPath, errorMessage) {
  ...
  if (!isPathInsideOrEqual(resolvedRoot, resolvedAncestor)) {
