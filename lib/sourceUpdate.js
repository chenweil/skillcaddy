...
export async function planLocalSourceUpdate({
  ...
  if (affectedProjectLinks.length > 0 && request.allowBreaking !== true) {
  ...
  updatePlanRequests.set(plan, {
    ...
    request: {
      sourceId,
      input: request.input,
      allowBreaking: request.allowBreaking === true
    }
  });
  return plan;
}

async function findAffectedProjectLinks({
  rootDir,
  projectPath,
  installPath,
  removedSkillPaths
...
export async function applyLocalSourceUpdate({
  ...
  const currentPlan = await planLocalSourceUpdate({
    rootDir,
    archiveLimits,
    projectPath
...
async function findUpdateRecord(rootDir, sourceId) {
  ...
  if (record.type !== 'local') {
    throw new SourceAcquisitionError(
      'unsupported-source',
      `Source update requires a Local source: ${sourceId}`
    );
  }
  return record;
}

async function assertActiveSourceMatchesRecord(destination, record) {
...
function classifySkillChanges(previousSkills, nextSkills) {
