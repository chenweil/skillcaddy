...
function parseCentralEntry(header, nameBuffer) {
  ...
  const encoding = flags & 0x800 ? 'utf8' : 'ascii';
  const name = decodeEntryName(nameBuffer, encoding);
  ...
  const entryKind = entryType === 0o040000 ||
  ...
  return {
    ...
    method,
    nameBuffer,
    path: validateEntryPath(name, entryKind),
    permissions: unixMode ? unixMode & 0o777 : entryKind === 'directory' ? 0o755 : 0o644,
...
async function validateOutputPaths(entries, destination, limits) {
  ...
  for (const entry of entries) {
    ...
    if (!isPathInsideOrEqual(path.resolve(destination), resolved)) {
...
function validateEntryPath(name, entryKind) {
  ...
  const withoutTrailingSlash = entryKind === 'directory' ? name.replace(/\/+$/, '') : name;
...
function decodeEntryName(buffer, encoding) {
  if (encoding === 'ascii' && buffer.some((byte) => byte > 0x7f)) {
