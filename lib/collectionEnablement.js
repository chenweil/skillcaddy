...
export async function executeCollectionEnablement(rootDir, input) {
  ...
  const projectPath = input.projectPath || rootDir;
  ...
  const plan = buildCollectionEnablePlan(state, skillIds, { scope });
  ...
  for (const skillId of plan.targetSkillIds) {
    ...
    try {
      const result = await enableProjectSkill(rootDir, {
        ...
        ...(input.hermesDir ? { hermesDir: input.hermesDir } : {}),
  ...
  const refreshed = scope !== 'project'
    ? { setups: [], reminders: [], refresh: { ok: true } }
    : await refreshSetupGuidance(rootDir, state.projectPath, plan.setups, input.globalDir, input.hermesDir);
  return {
    scope,
    plan,
    outcomes,
...
async function refreshSetupGuidance(rootDir, projectPath, plannedSetups, globalDir, hermesDir) {
  try {
    const state = await getState(rootDir, projectPath, { globalDir, hermesDir });
