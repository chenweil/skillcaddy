import {
  exportLibraryImage as exportLibraryImageWorkflow,
  importLibraryImage as importLibraryImageWorkflow
} from './libraryImage.js';
...
export async function planUpdateSource(context, request) {
  return planSourceUpgrade({
    ...
    hermesDir: context?.hermesDir,
    gitFetchRetryDelaysMs: context?.gitFetchRetryDelaysMs
  }, {
    request,
...
export async function updateGitSources(context, options) {
  ...
  return updateRegisteredGitSources({
    ...
    publishRecord: context?.publishRecord,
    gitFetchRetryDelaysMs: context?.gitFetchRetryDelaysMs,
    gitNetworkFailureLimit: context?.gitNetworkFailureLimit
  }, options);
}
...
export async function exportLibraryImage(context, filePath) {
  return exportLibraryImageWorkflow({ ...context, rootDir: requireRootDir(context) }, filePath);
}

export async function importLibraryImage(context, filePath, options) {
  return importLibraryImageWorkflow({ ...context, rootDir: requireRootDir(context) }, filePath, options);
}
