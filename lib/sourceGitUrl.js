...
import { isGitHubBlobSkillFileInput } from './sourceRemoteFilePolicy.js';

export function isGitSourceInput(input) {
  if (typeof input !== 'string') return false;
  const trimmed = input.trim();
  return isSshGitInput(trimmed) || /^https:\/\//i.test(trimmed);
}

export function isDefiniteGitSourceInput(input) {
  if (typeof input !== 'string') return false;
  const trimmed = input.trim();
  if (isSshGitInput(trimmed)) return true;
  if (isGitHubBlobSkillFileInput(trimmed)) return false;
  let url;
  try {
    url = new URL(trimmed);
  } catch {
